...
(*this function returns true if no other value from start.n to 9 works
for a location*)
...
(*This function returns the first value from start to 9 that works*)
...
(*This function finds the entry x moves before a location*)
...
(*This function finds the most previous unfixed location*)
...
let backTrack model =
  ...
  | false ->
    ...
    in
    {model with location = prev newBoard model.location}
    ...
    |false ->
      ...
      |false->
        ...
        |false ->
          ...
          let newValue = firstWork newBoard.(r).(c) newBoard r c in
          newBoard.(r).(c) <- {n = newValue; fixed = false};
          {model with board = newBoard; location = next location}
        |true ->
          newBoard.(r).(c) <- {n = 0; fixed = false};
          backTrack {model with board = newBoard}

(*This function makes the board so that if it 0, the entry is not fixed*)
